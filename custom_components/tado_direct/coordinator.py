"""Coordinator for the Tado Direct integration."""
...
from .tado_api import TadoApiError, TadoDirectAPI, TadoZone
...
type TadoDirectConfigEntry = ConfigEntry[TadoDirectDataUpdateCoordinator]


class TadoDirectDataUpdateCoordinator(DataUpdateCoordinator[dict[str, dict]]):
    """Class to manage API calls from and to Tado via TadoDirectAPI."""
    ...
    def __init__(
        ...
        self._default_overlays: dict[int, dict] = {}
    ...
    async def _async_update_data(self) -> dict[str, dict]:
        ...
        # Check if refresh token changed and update config entry
        current_refresh_token = self._tado.get_refresh_token()
        if current_refresh_token and current_refresh_token != self._refresh_token:
            ...
            self.hass.config_entries.async_update_entry(
                ...
                data={
                    **self.config_entry.data,
                    CONF_REFRESH_TOKEN: current_refresh_token,
                },
            )

        return self.data

    async def _async_fetch_default_overlays(self) -> None:
        """Fetch default overlay settings for each zone."""
        for zone in self.zones:
            zone_id = zone["id"]
            if zone_id not in self._default_overlays:
                try:
                    self._default_overlays[zone_id] = (
                        await self._tado.get_zone_overlay_default(zone_id)
                    )
                except (TadoApiError, aiohttp.ClientError):
                    _LOGGER.debug(
                        "Could not fetch default overlay for zone %s", zone_id
                    )
                    self._default_overlays[zone_id] = {}
        ...
        for device in devices:
            ...
            try:
                if (
                    ...
                    device[TEMP_OFFSET] = await self._tado.get_device_info(
    ...
    async def get_auto_geofencing_supported(self) -> bool:
        ...
        try:
            return await self._tado.get_auto_geofencing_supported()
        except (TadoApiError, aiohttp.ClientError) as err:
            raise UpdateFailed(f"Error updating Tado data: {err}") from err
    ...
    async def set_presence(
        ...
        if presence == PRESET_AWAY:
            await self._tado.set_away()
        elif presence == PRESET_HOME:
            await self._tado.set_home()
        elif presence == PRESET_AUTO:
            await self._tado.set_auto()
    ...
    async def set_meter_reading(self, reading: int) -> dict[str, Any]:
        ...
        try:
            return await self._tado.set_eiq_meter_readings(dt, reading)
        except (TadoApiError, aiohttp.ClientError) as err:
    ...
    async def set_child_lock(self, device_id: str, enabled: bool) -> None:
        ...
        except (TadoApiError, aiohttp.ClientError) as exc:
