"""Support for Tado Direct binary sensors for each zone."""
...
from .entity import TadoDirectDeviceEntity, TadoDirectZoneEntity

_LOGGER = logging.getLogger(__name__)


@dataclass(frozen=True, kw_only=True)
class TadoDirectBinarySensorEntityDescription(BinarySensorEntityDescription):
    """Describes Tado Direct binary sensor entity."""
...
BATTERY_STATE_ENTITY_DESCRIPTION = TadoDirectBinarySensorEntityDescription(
    key="battery state",
    state_fn=lambda data: data["batteryState"] == "LOW",
    device_class=BinarySensorDeviceClass.BATTERY,
)
CONNECTION_STATE_ENTITY_DESCRIPTION = TadoDirectBinarySensorEntityDescription(
    key="connection state",
    translation_key="connection_state",
    state_fn=lambda data: data.get("connectionState", {}).get("value", False),
    device_class=BinarySensorDeviceClass.CONNECTIVITY,
)
POWER_ENTITY_DESCRIPTION = TadoDirectBinarySensorEntityDescription(
    key="power",
    state_fn=lambda data: data.power == "ON",
    device_class=BinarySensorDeviceClass.POWER,
)
LINK_ENTITY_DESCRIPTION = TadoDirectBinarySensorEntityDescription(
    key="link",
    state_fn=lambda data: data.link == "ONLINE",
    device_class=BinarySensorDeviceClass.CONNECTIVITY,
)
OVERLAY_ENTITY_DESCRIPTION = TadoDirectBinarySensorEntityDescription(
    key="overlay",
...
OPEN_WINDOW_ENTITY_DESCRIPTION = TadoDirectBinarySensorEntityDescription(
    key="open window",
...
EARLY_START_ENTITY_DESCRIPTION = TadoDirectBinarySensorEntityDescription(
    key="early start",
...
async def async_setup_entry(
    ...
    """Set up the Tado Direct binary sensor platform."""
    ...
    for device in devices:
        ...
        entities.extend(
            [
                TadoDirectDeviceBinarySensor(tado, device, entity_description)
...
class TadoDirectDeviceBinarySensor(TadoDirectDeviceEntity, BinarySensorEntity):
    """Representation of a Tado Direct device binary sensor."""

    entity_description: TadoDirectBinarySensorEntityDescription
    ...
    def __init__(
        ...
        """Initialize of the Tado Direct binary sensor."""
...
class TadoDirectZoneBinarySensor(TadoDirectZoneEntity, BinarySensorEntity):
    """Representation of a Tado Direct zone binary sensor."""
